            ...
            <table class="adminlist" id="attrTable">
                <thead>
                    <tr>
                        ...
                        <th width="65%">
                            <?php echo JText::_('COL_TITLE_ATTR_VALUE'); ?>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                            foreach ($info as $key => $value) {
                                ...
                                if (substr($key, 0, 5) == 'attr_') {
                                    echo JText::_('TAGS_' . strtoupper($key));
                                } else {
                                    echo JText::_('' . strtoupper($key));
                                }
                                echo '</td>';
                                echo '<td>';
                                if ($key == 'status') {
                                    if (strlen($value) > 0)
                                        if (is_null($info['datasub']))
                                            echo '<i>' . JText::_('WAITING_OPTIN') . '</i>';
                                        else
                                            echo '<i>' . JText::_('WAITING_OPTIN_REMOVE') . '</i>';
                                    else
