...
class NumsStatistic extends NewsletterStatistic {
    ...
    function perform() {
        ...
        $query = "SELECT count(stat_id) as nunsubs " .
            "FROM `#__jinc_stats_event` " .
            "WHERE `stat_date` < '" . date($phplformat, $timeline->getStartTime()) . "' AND " .
            "`stat_type` = 1 AND " .
            "`stat_news_id` = " . (int) $this->getNewsId();
        ...
        $query = "SELECT date_format(`stat_date`, '" . $myformat . "') as statdate, count(1) as nsubs " .
            "FROM `#__jinc_stats_event` " .
            "WHERE `stat_date` >= '" . date($phplformat, $timeline->getStartTime()) . "' AND " .
        ...
        $query = "SELECT date_format(`stat_date`, '" . $myformat. "') as statdate, count(1) as nunsubs " .
            "FROM `#__jinc_stats_event` " .
            "WHERE `stat_date` >= '" . date($phplformat, $timeline->getStartTime()) . "' AND " .
            "`stat_date` <= '" . date($phprformat, $timeline->getEndTime()) . "' AND " .
            "`stat_type` = 1 AND " .
