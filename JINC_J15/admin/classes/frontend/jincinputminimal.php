...
class JINCInputMinimal extends JINCInputRenderer {
    ...
    public function modRender($attribute, $mandatory = false) {
        ...
        echo '   <img id="captcha" src="' . JRoute::_('index.php?option=com_jinc&controller=newsletter&task=showCaptcha&format=raw') . '" alt="CAPTCHA Image" />';
        echo '</td></tr>';
        echo '<tr><td>';
        echo JText::_('INPUT_CAPTCHA') . '*';
        echo '</td></tr>';        
        echo '<tr><td>';
        echo '    <input type="text" name="captcha_code" size="10" maxlength="6" />&nbsp;&nbsp;';
        echo '    <img alt="' . JText::_('ALT_CAPTCHA') . '" width="16px" height="16px" onclick="document.getElementById(\'captcha\').src = \'index.php?option=com_jinc&controller=newsletter&task=showCaptcha&format=raw & \' + Math.random(); return false" src="components/com_jinc/securimage/images/refresh.gif" />';
        echo '</td></tr>';
    }
    ...
    public function modCaptchaRender() {
        ...
        echo '      <img id="mod_captcha" src="' . JRoute::_('index.php?option=com_jinc&controller=newsletter&task=showCaptcha&format=raw&mod_jinc=true') . '" alt="CAPTCHA Image" />';
        echo '  </div>';
        echo '</td></tr>';
        $onfocus = 'this.value=\'\'; this.setAttribute(\'class\', \'selected\');';
        $label = JText::_('INPUT_CAPTCHA') . '*';
        echo '<tr><td>';
        echo '  <input value="' . $label . '" class="unselected" onfocus="' . $onfocus . '" type="text" name="captcha_code" size="10" maxlength="6" />&nbsp;&nbsp;';
        echo '  <img alt="' . JText::_('ALT_CAPTCHA') . '" width="16px" height="16px" onclick="document.getElementById(\'mod_captcha\').src = \'index.php?option=com_jinc&controller=newsletter&task=showCaptcha&format=raw&mod_jinc=true&\' + Math.random(); return false" src="components/com_jinc/securimage/images/refresh.gif" />';
        echo '</td></tr>';
