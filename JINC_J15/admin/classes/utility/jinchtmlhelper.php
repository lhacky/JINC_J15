...
class JINCHTMLHelper {
    ...
    function hint($hint_name, $hint_title, $hint_header = '',
        $hint_footer = '') {
        ...
        echo '<legend><img src="images/checkin.png" border="0" align="absmiddle" alt="Hint" width="24px" height="24px" hspace="6">';
        echo '<strong>&nbsp;&nbsp;' . JText::_('HINT_' . $hint_title) . ' </strong></legend>';
        echo $hint_header;
        echo JText::_('HINT_' . $hint_name);
        echo $hint_footer;
        echo '</fieldset></center></div>';
    }

    function legend($legend_array, $img_base = 'images/') {
        echo '<table cellspacing="0" cellpadding="4" align="center">';
        echo '<tr align="center">';
        for ($i = 0 ; $i < count($legend_array) ; $i++) {
            $element = $legend_array[$i];
            $text = JText::_('LEGEND_' . $element['text']);
            $icon = $element['icon'];
    ...
    function statChart() {
        ...
        if ($chart_system == STATSRENDERER_BUILTIN) {
            echo '<img align="center" src="index.php?option=com_jinc&controller=newsletter&task=statsRender&format=raw" alt="Chart">';
        } else {
