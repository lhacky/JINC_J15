 ...
 */
    ...
    function getSubscribersList($start_time = 0, $start_id = 0, $multiplexer = 1, $attributes = null) {
        ...
        $query = 'SELECT u.email as usermail, u.username as userid, u.name as username, ' .
            'UNIX_TIMESTAMP(s.subs_datasub) as last_time, s.subs_id as last_id ';
        ...
        foreach ($attributes as $attr_name => $attr_value) {
            ...
            ' ON s.subs_id = ' . $table_alias . '.id AND n.news_id = ' . $table_alias . '.news_id ';
            $i++;
        }

        $query .= 'WHERE n.news_id = ' . (int) $this->getNewsId() . ' ' .
            'AND ( UNIX_TIMESTAMP(s.subs_datasub) > ' . (int) $start_time . ' ' .
            'OR (UNIX_TIMESTAMP(s.subs_datasub) = ' . (int) $start_time . ' AND s.subs_id > ' . (int) $start_id . ') ) ' . ' ' .
            'ORDER BY s.subs_datasub, s.subs_id ' .
    ...
    function getSubscriber($id = 0, $attributes = null) {
        ...
        $query = 'SELECT u.email as email, u.id as user_id, u.username as username, u.name as name, s.subs_datasub as datasub ';
        ...
        $query .= 'WHERE n.news_id = ' . (int) $this->getNewsId() . ' AND s.subs_id = ' . (int) $id;
