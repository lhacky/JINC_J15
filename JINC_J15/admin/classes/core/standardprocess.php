    ...
    function play($client_id = '') {
        ...
        if ($send_result = $message->send($this->last_subscriber_time, $this->last_subscriber_id)) {
            ...
            $query = 'UPDATE #__jinc_process ' .
                'SET proc_last_subscriber_time = FROM_UNIXTIME(' . $last_time . '), ' .
                'proc_last_update_time = NOW(), ' .
                'proc_last_subscriber_id = ' . $last_id . ', ' .
                'proc_sent_success = ' . $this->sent_success . ', ' .
                'proc_sent_messages = ' . $this->sent_messages . ' ';
            if ($start_time > 0)
                $query .= ', proc_start_time = FROM_UNIXTIME(' . $start_time . ') ';
            $query .= 'WHERE proc_id = ' . $this->id;

            $dbo->setQuery($query);
            $logger->debug('Process: executing query: ' . $query);
            if (!$dbo->query()) {
                $this->setError('ERR039');
                return false;
            }
