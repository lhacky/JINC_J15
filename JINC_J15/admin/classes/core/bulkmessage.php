...
class BulkMessage extends Message {
    ...
    function send($start_time = 0, $start_id = 0) {
        ...
        if ($newsletter = $this->loadNewsletter()) {
            ...
            for ($i = 0; $i < $nrecips; $i += $max_bulk_bcc) {
                ...
                for ($j = 0; $j < $max_bulk_bcc && ($j + $i) < $nrecips; $j++) {
                    ...
                    $mailmsg->addBCC($current['usermail']);
                    $ndest = $ndest + 1;
                ...
                if ($send_mail) {
                    $nsuccess = $nsuccess + $ndest * $mailmsg->send();
                } else {
                    $bcc_addresses = array();
        ...
        /*
          ...
          if ($start == 0) {
          $logger->finer('BulkMessage: triggering statistical sent event');
          $dispatcher = &JDispatcher::getInstance();
          $params = array('news_id' => $this->get('news_id'));
          $result = $dispatcher->trigger('jinc_sent', $params);

          $logger->finer('BulkMessage: triggering message sent event');
          $params = array('news_id' => $this->get('news_id'),
          'msg_id' => $this->get('id'));
          $result = $dispatcher->trigger('jinc_sent', $params);
          }
