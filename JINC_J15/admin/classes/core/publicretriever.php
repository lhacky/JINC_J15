 ...
 */
    ...
    function getSubscribersList($start_time = 0, $start_id = 0, $multiplexer = 1, $attributes = null) {
        ...
        if (is_null($attributes) || ! is_array($attributes)) $attributes = array();

        $query = 'SELECT p.pub_email as usermail, ' . 
            'UNIX_TIMESTAMP(p.pub_datasub) as last_time, p.pub_id as last_id';
        ...
        $query .= ' FROM #__jinc_newsletter n '.
            'LEFT JOIN #__jinc_public_subscriber p ON p.pub_news_id = n.news_id ';
        ...
        foreach ($attributes as $attr_name => $attr_value) {
            ...
            $query .= ' LEFT JOIN ' . $table_name . ' ' . $table_alias . 
                ' ON p.pub_id = ' . $table_alias . '.id AND n.news_id = ' . $table_alias . '.news_id ';
            $i++;
        }

        $query .= 'WHERE n.news_id = ' . (int) $this->getNewsId() . ' ' .
            'AND (p.pub_random = \'\' OR p.pub_random IS NULL) ' .
            'AND ( UNIX_TIMESTAMP(p.pub_datasub) > ' . (int) $start_time . ' ' .
            'OR (UNIX_TIMESTAMP(p.pub_datasub) = ' . (int) $start_time . ' AND p.pub_id > ' . (int) $start_id . ') ) ' . ' ' .
            'ORDER BY p.pub_datasub, p.pub_id ' .
    ...
    function getSubscriber($id = 0, $attributes = null) {
        ...
        $query = 'SELECT p.pub_random as status, p.pub_email as usermail, ' .
            'p.pub_datasub as datasub';
        $i = 0;
        foreach ($attributes as $attr_name => $attr_value) {
            $table_name = '`#__jinc_attribute_' . $attr_name . '`';
            $table_alias = 'att' . $i;
        ...
        $query .= 'WHERE n.news_id = ' . (int) $this->getNewsId() . ' AND p.pub_id = ' . (int) $id;
    ...
    function countSubscribers() {
        ...
        $dbo =& JFactory::getDBO();
        $query = 'SELECT COUNT(p.pub_email) AS num_subs ' .
            'FROM #__jinc_newsletter n '.
            'LEFT JOIN #__jinc_public_subscriber p ON p.pub_news_id = n.news_id '.
            'WHERE n.news_id = ' . (int) $this->getNewsId() . ' ' .
            'AND (p.pub_random = \'\' OR p.pub_random IS NULL)';
        $dbo->setQuery($query);
        $logger->debug('PublicRetriever: executing query ' . $query);
