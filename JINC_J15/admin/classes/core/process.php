 ...
 */
    ...
    function updateStatus($status, $client_id = '') {
        if ($this->status != PROCESS_STATUS_FINISHED) {
            ...
            $query = 'UPDATE #__jinc_process ' .
                'SET proc_status = ' . $status . ' ';
            if ($client_id != '')
                $query .= ', proc_client_id = \'' . $client_id . '\' ';

            if ($status == PROCESS_STATUS_STOPPED) {
                $query .= ', proc_sent_success = 0 ';
                $query .= ', proc_sent_messages = 0 ';
            }
            $query .= 'WHERE proc_id = ' . $this->get('id');
            $dbo->setQuery($query);
    ...
    function reloadStatus() {
        ...
        $logger = $servicelocator->getLogger();
        $dbo = & JFactory::getDBO();
        $query = 'SELECT proc_status FROM #__jinc_process ' .
            'WHERE proc_id = ' . $this->get('id');
        $dbo->setQuery($query);
        ...
        if ($result = $dbo->loadAssocList()) {
            ...
            $this->status = $result[0]['proc_status'];
        } else {
            return false;
        }
        return true;
