...
class SubscriptionNotifyEvent extends JObserver {
/**
 * Constructor
 *
 * @access	protected
 */
    ...
    function update($args) {
        ...
        if (isset($args['news_notify']) &&  $args['news_notify']) {
            ...
            $query = 'SELECT name, email, sendEmail FROM #__users ' .
            ...
            if ($rows = $dbo->loadObjectList()) {
                ...
                $subject = sprintf ( JText::_( 'MAIL_NOTIFY_SUBJECT' ), $news_name);
                $subject = html_entity_decode($subject, ENT_QUOTES);

                foreach ( $rows as $row ) {
                    if ($row->sendEmail) {
                        $body = sprintf ( JText::_( 'MAIL_NOTIFY_BODY' ), $row->name, $news_name, $subs_name);
